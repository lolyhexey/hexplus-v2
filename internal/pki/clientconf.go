...
// every per-user .ovpn export.
//
// Same trim as serverconf.go: no comp-lzo, no client-cert-not-required,
// data-ciphers wired up. The {{.RemoteHost}}/{{.RemotePort}}/{{.Proto}}
// placeholders are filled in by the user export step at runtime; we
// don't bake an IP into the binary because the server's public address
// is per-deployment.
...
// RenderClientCommon emits just the [Header] portion of a .ovpn file -
...
const clientCommonTmpl = `# Generated by hexplus user export.
client
dev tun
proto {{.Proto}}
...
# Cipher set covers OpenVPN 2.5+ (data-ciphers negotiation) and the
# CBC fallback for older client apps that still ship 2.4 forks.
